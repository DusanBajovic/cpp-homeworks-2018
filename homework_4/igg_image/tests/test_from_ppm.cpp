...
TEST(TestImage, ReadingFromPPMFile) {
  std::shared_ptr<igg::IoStrategy> strategy = std::make_shared<igg::PpmIoStrategy>();
  igg::Image image;
  image.SetIoStrategy(strategy);

  EXPECT_EQ(0, image.ReadFromDisk("../data/pbmlib.ascii.ppm"));
...
TEST(TestImage, WritingToPPMFiles) {
  ...
  EXPECT_EQ(0, image.ReadFromDisk("../data/pbmlib.ascii.ppm"));
  EXPECT_EQ(0, image.WriteToDisk("../data/pbmlib.ascii_copy.ppm"));
}

TEST(TestImage, WritingAndReadingToPPMFiles) {
  std::shared_ptr<igg::IoStrategy> strategy = std::make_shared<igg::PpmIoStrategy>();
  igg::Image write_image(4, 4);
  igg::Image read_image;
  write_image.SetIoStrategy(strategy);
  read_image.SetIoStrategy(strategy);
  ...
  EXPECT_EQ(0, write_image.WriteToDisk("../data/custom.ppm"));
  EXPECT_EQ(0, read_image.ReadFromDisk("../data/custom.ppm"));
